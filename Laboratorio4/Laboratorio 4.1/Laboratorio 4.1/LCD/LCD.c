...
#define RS PB0   // REGISTRO DE SELECCIÓN (RS) EN EL PIN PD2
#define RW PB1   // LECTURA/ESCRITURA (RW) EN EL PIN PD3
#define E  PB2   // HABILITACIÓN (E) EN EL PIN PB4
...
void lcd_command(unsigned char cmd) {
	LCD_PORTB &= ~(1 << RS); // RS = 0, INDICA QUE ES UN COMANDO
	LCD_PORTB &= ~(1 << RW); // RW = 0, INDICA QUE ES UNA ESCRITURA
	
	LCD_PORT = (cmd & 0x0F);						// ENVÍA LOS 4 BITS MENOS SIGNIFICATIVOS A PORTB
...
void lcd_init(void) {
	...
	LCD_DDRB = 0x07;	// CONFIGURA PB0-PB3 COMO SALIDA
...
void lcd_data(unsigned char data) {
	LCD_PORTB |= (1 << RS);  // RS = 1, INDICA QUE ES UN DATO (NO COMANDO)
	LCD_PORTB &= ~(1 << RW); // RW = 0, INDICA QUE ES UNA ESCRITURA
	
	LCD_PORT = (data & 0x0F); // ENVÍA LOS 4 BITS MENOS SIGNIFICATIVOS A PORTB
